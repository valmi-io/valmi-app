...
import { redirectToCreateDestinationWarehouse, redirectToCreateStream } from '@/utils/router-utils';
...
const EventsPage: NextPageWithLayout = () => {
  ...
  const handleConnectionOnClick = ({ connectionType, id }: { connectionType: EventConnectionType; id: string }) => {
    if (connectionType === 'STREAM') {
      router.push(`${getBaseRoute(workspaceId)}/events/streams?id=${id}`);
    } else if (connectionType === 'DESTINATION') {
      router.push(`${getBaseRoute(workspaceId)}/events/destination-warehouses?id=${id}`);
    }
  };
    ...
    if (isDataEmpty(links)) {
      ...
      const handleOnClick = () => {
        if (connectionType === 'STREAM') {
          ...
          redirectToCreateDestinationWarehouse({ router: router, wid: workspaceId });
        }
      };
  ...
  return (
    <PageLayout pageHeadTitle="Events" title="EVENTS" buttonTitle="EVENT" displayButton={false}>
      <ContentLayout
        key={`eventsPage`}
        error={streamsError ?? destinationsError ?? linksError}
