...
const DestinationsPage: NextPageWithLayout = () => {
  ...
  const handleButtonOnClick = ({ edit = false, id = '', type = '', supertype = '' }) => {
    ...
    if (edit) {
      router.push(`${getBaseRoute(workspaceId)}/events/destination-warehouses/create/${type}`);
    } else {
      router.push(`${getBaseRoute(workspaceId)}/events/destination-warehouses/create`);
    }
  };
...
const PageContent = ({ data, id, handleButtonOnClick }: { data: TData; id: string; handleButtonOnClick: any }) => {
  if (isDataEmpty(data)) {
    return <ListEmptyComponent description={'No destination warehouses found in this workspace'} />;
  }
  return <DestinationsTable data={data} id={id} handleButtonOnClick={handleButtonOnClick} />;
};
